...
using namespace std;
string str="{\"refresh_interval\":60,"
...
"]"
"}";
const char* configFile="configs.json";
int main()
{
	int iValue;
	double fTimeout;
	std::string strValue;
	neb::CJsonObject* oJson=new neb::CJsonObject(str);
	...
	while(buf && ifile.get(ch))
		if(ch!='\n' && ch!='\t')
			buf.put(ch);
	cout<<"config file sting :"<<buf.str()<<endl;
	neb::CJsonObject* jsonObj=new neb::CJsonObject(buf.str());
	std::cout << "-------------------------------------------------------------------" << std::endl;
	cout <<(*jsonObj).ToString()<<endl;
	string versionStr;
	(*jsonObj).Get("version",versionStr);
	std::cout << "version :" <<versionStr << std::endl;
//	std::cout << (*oJson)("date") << std::endl;
